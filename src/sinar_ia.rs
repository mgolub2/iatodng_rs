...
impl SinarIAMeta {
    pub fn process_meta(meta: &[u8]) -> Self {
        ...
        let focal_length =
            (u32::from_le_bytes(meta[356..360].try_into().unwrap()) as f32 / 1000.0).round() as u32;
...
//unused
#[allow(dead_code)]
fn buffer_to_2d_array(buffer: &Vec<u8>, width: usize, height: usize) -> Array2<u16> {
...
fn buffer_to_2d_array_f64(buffer: &Vec<u8>, width: usize, height: usize) -> Array2<f64> {
    assert_eq!(buffer.len(), width * height * 2);
...

    {
        ...
        for (i, chunk) in buffer.chunks_exact(2).enumerate() {
            let row = i / width;
            let col = i % width;
            let value = u16::from_le_bytes(chunk.try_into().unwrap()) as f64 / 65535.0;
            array_view[[row, col]] = value;
        }
    }
...
fn subract_black_ref_mut(
    ...
    black_ref0: &Array2<f64>,
    black_ref1: &Array2<f64>,
) {
    assert_eq!(image.shape(), black_ref0.shape());
    {
        Zip::from(image)
...
fn apply_white_ref_mut(image: &mut Array2<f64>, white_ref: &Array2<f64>) {
...
fn write_2d_array_to_dng(image: &Array2<f64>, path: &PathBuf, meta: &SinarIAMeta) {
    // let file = File::create(path).unwrap();
    // let mut ifd = Ifd::new(IfdType::Ifd);
    // ifd.insert(tags::ifd::DNGVersion, &[1u8, 2, 0, 0]);
    // ifd.insert(tags::ifd::UniqueCameraModel, meta.camera.to_string());
    // //let mut ifd1 = Ifd::new(IfdType::Ifd);

    // //ifd.insert(tags::ifd::NewSubfileType, 0u32);
    // ifd.insert(tags::ifd::ImageLength, meta.height);
    // ifd.insert(tags::ifd::ImageWidth, meta.width);
    // ifd.insert(tags::ifd::BitsPerSample, 16u16);
    // ifd.insert(tags::ifd::PhotometricInterpretation, 32803u16);
    // ifd.insert(tags::ifd::RowsPerStrip, meta.height);
    // ifd.insert(tags::ifd::StripByteCounts, 2 * meta.width*meta.height);
    // ifd.insert(tags::ifd::CFALayout, 1u16);
    // ifd.insert(tags::ifd::CFAPlaneColor, &[0u8, 1, 2]);
    // ifd.insert(tags::ifd::CFAPattern, &[1u8, 0, 2, 1]);
    // ifd.insert(tags::ifd::CFARepeatPatternDim, &[2u16, 2u16]);
    // ifd.insert(tags::ifd::SamplesPerPixel, 1u16);
    // ifd.insert(tags::ifd::PlanarConfiguration, 1u16);


    //ifd.insert(tags::ifd::StripOffsets, 2048);
    // ifd.insert(
    //     tags::ifd::StripOffsets,
    //     IfdValue::Offsets(Arc::new(
    //         image
    //             .iter()
    ...
    //let res = DngWriter::write_dng(file, true, FileType::Dng, vec![ifd]);
    //let dng_img = rawloader::decode(image);
    // let file = File::create("test.dng").unwrap();
    // let mut ifd = Ifd::new(IfdType::Ifd);
    // ifd.insert(Copyright, "this is a test string");
    // ifd.insert(CFAPattern, &[0u8, 1, 0, 2]);
    // ifd.insert(StripOffsets, IfdValue::Offsets(Arc::new(vec![0u8, 0, 0, 0])));
    // ifd.insert(StripByteCounts, 4u16);
    // ifd.insert(DNGVersion, &[1u8, 2, 0, 0]);
    // let res = DngWriter::write_dng(file, true, FileType::Dng, vec![ifd]);
    if let Err(e) = res {
        println!("Error writing DNG: {}", e);
    }
}

pub fn process_ia(path: &PathBuf) -> Option<()> {
    ...
    metadata
        ...
        .and_then(|meta| {
            ...
            println!("Processing IA: {}...\n\tblack: {}\n\twhite: {}", path.to_str().unwrap(), black_full_path.to_str().unwrap(), white_full_path.to_str().unwrap());
            pwad::Pwad::from_file(black_full_path.to_str().unwrap()).and_then(|black| {
                Ok({
                    let mut raw = buffer_to_2d_array_f64(
                        &metadata.read_lump_by_tag(RAW_KEY)?,
                        ia.width as usize,
                        ia.height as usize,
                    );
                    let black_ref0 = buffer_to_2d_array_f64(
                        &black.read_lump_by_tag(BLACK0_KEY)?,
                        ia.width as usize,
                        ia.height as usize,
                    );
                    let black_ref1 = buffer_to_2d_array_f64(
                    ...
                    pwad::Pwad::from_file(white_full_path.to_str().unwrap())
                        ...
                        .ok();
                    write_2d_array_to_dng(&raw, &PathBuf::from("test.dng"), &ia)
                })
            })
